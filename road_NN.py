...
lane lines on a regular road image. This file currently contains
the necessary code to 1) load road images and labels pickle files,
2) shuffle and then splits the data into training and validation
sets, 3) creates the neural network architecture, 4) train the network,
5) save down the model architecture and weights, 6) show the model summary.
"""

# Load training images
train_images = pickle.load(open("full_road_train.p", "rb" ))
...
from keras.layers import Convolution2D, Flatten, MaxPooling2D, Cropping2D
...
# To crop the top 1/3 of the image below to assist in the NN determining important pixels
crop_amount = int(X_train.shape[1]/3)

# Here is the actual neural network
model = Sequential()
# Normalizes incoming inputs. First layer needs the input shape to work
model.add(BatchNormalization(input_shape=input_shape))
# Crops from the top of the image
# This version of Keras had a bug with cropping zeroes so set others to '1'
model.add(Cropping2D(cropping=((crop_amount,1),(1,1))))
...
with open("road_model.json", "w") as json_file:
    json_file.write(model_json)

model.save_weights('road_model.h5')
